...
function dragDrop() {
	...
	for (let i = 0; i < cartCheck.length; i++) {
		if (dragItem.src === cartCheck[i].src) {
			...
			totalCartPrice += Number(books[booksItem]["price"]); //TOTALSEARCH
	...
	const total = document.querySelector('.total__price'); //TOTALSEARCH
	total.textContent = totalCartPrice;
}
...
function deleteBook() {
	const minus = Number(this.parentElement.querySelector('.cart__price').textContent); //TOTALSEARCH
	const total = document.querySelector('.total__price');
